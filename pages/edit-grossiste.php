...
$id = (int) ($_GET['id'] ?? 0);
if (!$id) {
    header('Location: grossistes.php');
    exit;
}
...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM grossistes_dpm WHERE id = ?");
    $stmt->execute([$id]);
    $grossiste = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    $grossiste = null;
...
if (!$grossiste) {
    header('Location: grossistes.php?toast=not_found');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $old = array_merge($grossiste, $_POST);
    $old['id'] = $id;
    ...
    try {
        ...
        $success = true;
        $st2 = $pdo->prepare("SELECT * FROM grossistes_dpm WHERE id = ?");
        $st2->execute([$id]);
        $grossiste = $st2->fetch(PDO::FETCH_ASSOC);
        $old = $grossiste;
...
<head>
    ...
    <title>Modifier — <?= htmlspecialchars($grossiste["nom_grossiste"]) ?> — DPM Archive</title>
...
<body>
    ...
    <div class="sb-main">
        <header class="topbar">
            ...
            <h1 class="topbar__title">Modifier — <?= htmlspecialchars($grossiste["nom_grossiste"]) ?></h1>
            <span class="topbar__meta"><?= date('d/m/Y') ?></span>
        </header>

        <div class="form-page">
            <div class="form-page__header">
                <h2 class="form-page__title">Modifier le grossiste</h2>
                ...
                <div class="form-layout">
                                ...
                                <div class="fields-grid">
                                    <div class="field field-full">
                                        ...
                                        <?php if (isset($errors['nom_grossiste'])): ?>
                                            <div class="field__error visible"><?= $errors['nom_grossiste'] ?></div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <div class="fields-grid fields-grid--2" style="margin-top:16px">
                                    <div class="field">
                                        ...
                                        <?php if (isset($errors['responsable'])): ?>
                                            <div class="field__error visible"><?= $errors['responsable'] ?></div>
                                        <?php endif; ?>
                                    </div>
                                    <div class="field">
                                        ...
                                        <?php if (isset($errors['telephone'])): ?>
                                            <div class="field__error visible"><?= $errors['telephone'] ?></div>
                                        <?php endif; ?>
                                    </div>
                                    <div class="field field-full">
                            ...
                            <div class="form-card__body">
                                ...
                                <div class="toggle-group" style="margin-top:16px">
                                    <label class="toggle-item"><input type="checkbox" name="is_actif"
                                        ...
                                        <div class="toggle-switch"></div><span class="toggle-label">Grossiste
                                            actif</span>
                                    </label>
                                </div>
                            </div>
                    ...
                    <div class="form-sidebar">
                        ...
                        <div class="form-sidebar__actions">
                            ...
                            <a href="grossistes.php" class="btn-cancel">Annuler</a>
                        </div>
                    </div>
                </div>
            </form>
