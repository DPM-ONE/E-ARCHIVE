...
if (!in_array($_SESSION['role'] ?? '', ['super_admin','admin'])) {
    header('Location: depots.php?toast=permission_denied'); exit;
}

$id = (int)($_GET['id'] ?? 0);
if (!$id) { header('Location: depots.php'); exit; }
...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM depots_dpm WHERE id = ?");
    $stmt->execute([$id]);
...
if (!$depot) { header('Location: depots.php?toast=not_found'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $success = true;
        $st2 = $pdo->prepare("SELECT * FROM depots_dpm WHERE id = ?");
        $st2->execute([$id]);
...
<head>
    ...
    <title>Modifier — <?= htmlspecialchars($depot["depot_pharmaceutique"]) ?> — DPM Archive</title>
...
<header class="topbar">
    ...
    <a href="depots.php" class="topbar__back">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="15 18 9 12 15 6"/></svg>Dépôts
    </a>
    <h1 class="topbar__title">Modifier — <?= htmlspecialchars($depot["depot_pharmaceutique"]) ?> pharmaceutique</h1>
    <span class="topbar__meta"><?= date('d/m/Y') ?></span>
</header>

<div class="form-page">
    <div class="form-page__header">
        <h2 class="form-page__title">Modifier le dépôt pharmaceutique</h2>
        ...
        <div class="form-layout">
            ...
            <div>
                ...
                <div class="form-card">
                    <div class="form-card__header">
                        ...
                        <div><div class="form-card__title">Identité du dépôt</div><div class="form-card__sub">Nom et numéro de décision</div></div>
                    </div>
                    <div class="form-card__body">
                        <div class="fields-grid">
                            <div class="field field-full">
                                <label class="field__label required">Nom du dépôt pharmaceutique</label>
                                <input type="text" name="depot_pharmaceutique" class="field__input <?= isset($errors['depot_pharmaceutique'])?'error':'' ?>" placeholder="Ex : Dépôt Central Brazzaville" value="<?= htmlspecialchars($old['depot_pharmaceutique']??'') ?>" required>
                                <?php if(isset($errors['depot_pharmaceutique'])): ?><div class="field__error visible"><?= $errors['depot_pharmaceutique'] ?></div><?php endif; ?>
                            </div>
                            <div class="field field-full">
                    ...
                    <div class="form-card__body">
                        <div class="fields-grid fields-grid--2">
                            <div class="field">
                                ...
                                <input type="text" name="prenom" class="field__input <?= isset($errors['prenom'])?'error':'' ?>" placeholder="Prénom" value="<?= htmlspecialchars($old['prenom']??'') ?>" required>
                            ...
                            <div class="field">
                                ...
                                <input type="text" name="nom" class="field__input <?= isset($errors['nom'])?'error':'' ?>" placeholder="Nom de famille" value="<?= htmlspecialchars($old['nom']??'') ?>" required>
                ...
                <div class="form-card">
                    ...
                    <div class="form-card__body">
                        ...
                        <div class="toggle-group" style="margin-top:16px">
                            <label class="toggle-item"><input type="checkbox" name="is_active" <?= $old['is_active']?'checked':'' ?>><div class="toggle-switch"></div><span class="toggle-label">Dépôt actif</span></label>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="form-sidebar">
                ...
                <div class="form-sidebar__actions">
                    ...
                    <a href="depots.php" class="btn-cancel">Annuler</a>
                </div>
            </div>
        </div>
    </form>
...
const requiredFields = ['depot_pharmaceutique','prenom','nom','numero_decision','adresse','localite','departement'];
function updateCompletion() {
    ...
    // Live preview
    const nom = document.querySelector('[name=depot_pharmaceutique]')?.value || '';
