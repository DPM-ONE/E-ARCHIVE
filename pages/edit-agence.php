...
if (!in_array($_SESSION['role'] ?? '', ['super_admin','admin'])) {
    header('Location: agences.php?toast=permission_denied'); exit;
}

$id = (int)($_GET['id'] ?? 0);
if (!$id) { header('Location: agences.php'); exit; }
...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM agences_dpm WHERE id = ?");
    $stmt->execute([$id]);
    $depot = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    $depot = null; $depts = $allArrs = $allDists = []; $deptsWithArr = [];
}

if (!$depot) { header('Location: agences.php?toast=not_found'); exit; }

$old = $depot;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $old = array_merge($depot, $_POST);
    $old['id'] = $id;

    $required = ['nom_agence','responsable_prenom','responsable_nom','adresse','localite','departement'];
    ...
    try {
        ...
        $stmt->execute([
            ...
            ':responsable_prenom'   => trim($_POST['responsable_prenom']),
            ':responsable_nom'      => trim($_POST['responsable_nom']),
            ':numero_decision'      => trim($_POST['numero_decision'] ?? '') ?: null,
        ...
        $success = true;
        $st2 = $pdo->prepare("SELECT * FROM agences_dpm WHERE id = ?");
        $st2->execute([$id]);
        $depot = $st2->fetch(PDO::FETCH_ASSOC);
        $old = $depot;
...
<head>
    ...
    <title>Modifier — <?= htmlspecialchars($depot["nom_agence"]) ?> — DPM Archive</title>
...
<header class="topbar">
    ...
    <a href="agences.php" class="topbar__back">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="15 18 9 12 15 6"/></svg>Agences
    </a>
    <h1 class="topbar__title">Modifier — <?= htmlspecialchars($depot["nom_agence"]) ?> pharmaceutique</h1>
    <span class="topbar__meta"><?= date('d/m/Y') ?></span>
</header>

<div class="form-page">
    <div class="form-page__header">
        <h2 class="form-page__title">Modifier le agence pharmaceutique</h2>
        <p class="form-page__sub">Modifiez les informations ci-dessous</p>
    </div>

    <?php if ($success): ?>
    <div class="form-notif form-notif--success visible">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>
        Agence modifié avec succès !
        <a href="agences.php" style="margin-left:auto;color:inherit;font-weight:700;text-decoration:none;border-bottom:1px solid currentColor">Voir la liste →</a>
    </div>
    <?php endif; ?>
        ...
        <div class="form-layout">
            ...
            <div>
                ...
                <div class="form-card">
                    <div class="form-card__header">
                        ...
                        <div><div class="form-card__title">Identité du agence</div><div class="form-card__sub">Nom et numéro de décision</div></div>
                    </div>
                    <div class="form-card__body">
                        <div class="fields-grid">
                            <div class="field field-full">
                                <label class="field__label required">Nom du agence pharmaceutique</label>
                                <input type="text" name="nom_agence" class="field__input <?= isset($errors['nom_agence'])?'error':'' ?>" placeholder="Ex : Agence Central Brazzaville" value="<?= htmlspecialchars($old['nom_agence']??'') ?>" required>
                                <?php if(isset($errors['nom_agence'])): ?><div class="field__error visible"><?= $errors['nom_agence'] ?></div><?php endif; ?>
                            </div>
                            <div class="field field-full">
                                <label class="field__label required">Numéro de décision</label>
                                <input type="text" name="numero_decision" class="field__input <?= isset($errors['numero_decision'])?'error':'' ?>" placeholder="Ex : DEC-2024-001" value="<?= htmlspecialchars($old['numero_decision']??'') ?>" required>
                                <?php if(isset($errors['numero_decision'])): ?><div class="field__error visible"><?= $errors['numero_decision'] ?></div><?php endif; ?>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-card">
                    <div class="form-card__header">
                        <div class="form-card__icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>
                        <div><div class="form-card__title">Propriétaire</div><div class="form-card__sub">Informations du titulaire</div></div>
                    </div>
                    <div class="form-card__body">
                        <div class="fields-grid fields-grid--2">
                            <div class="field">
                                <label class="field__label required">Prénom du responsable</label>
                                <input type="text" name="responsable_prenom" class="field__input <?= isset($errors['prenom'])?'error':'' ?>" placeholder="Prénom" value="<?= htmlspecialchars($old['responsable_prenom']??'') ?>" required>
                                <?php if(isset($errors['prenom'])): ?><div class="field__error visible"><?= $errors['prenom'] ?></div><?php endif; ?>
                            </div>
                            <div class="field">
                                <label class="field__label required">Nom du responsable</label>
                                <input type="text" name="responsable_nom" class="field__input <?= isset($errors['nom'])?'error':'' ?>" placeholder="Nom de famille" value="<?= htmlspecialchars($old['responsable_nom']??'') ?>" required>
                                <?php if(isset($errors['nom'])): ?><div class="field__error visible"><?= $errors['nom'] ?></div><?php endif; ?>
                            </div>
                            <div class="field">
                                <label class="field__label">Téléphone</label>
                                <input type="tel" name="telephone" class="field__input" placeholder="+242 06 000 0000" value="<?= htmlspecialchars($old['telephone']??'') ?>">
                            </div>
                            <div class="field">
                                <label class="field__label">Email</label>
                                <input type="email" name="email" class="field__input" placeholder="contact@depot.com" value="<?= htmlspecialchars($old['email']??'') ?>">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="form-card__body">
                        <div class="fields-grid">
                            <div class="field field-full">
                                ...
                                <input type="text" name="adresse" class="field__input <?= isset($errors['adresse'])?'error':'' ?>" placeholder="Rue, avenue..." value="<?= htmlspecialchars($old['adresse']??'') ?>" required>
                            ...
                            <div class="field field-full">
                                ...
                                <input type="text" name="localite" class="field__input <?= isset($errors['localite'])?'error':'' ?>" placeholder="Ville ou commune" value="<?= htmlspecialchars($old['localite']??'') ?>" required>
                                <?php if(isset($errors['localite'])): ?><div class="field__error visible"><?= $errors['localite'] ?></div><?php endif; ?>
                            </div>
                        </div>
                        <div class="fields-grid fields-grid--2" style="margin-top:16px">
                            <div class="field">
                                ...
                                <div class="field__select-wrap">
                                    <select name="departement" id="selectDept" class="<?= isset($errors['departement'])?'error':'' ?>" required>
                                        <option value="">— Sélectionner —</option>
                                        <?php foreach($depts as $d): ?>
                                            <option value="<?= $d['id'] ?>" <?= (int)($old['departement_id']??0)==(int)$d['id']?'selected':'' ?>><?= htmlspecialchars($d['libelle']) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                ...
                <div class="form-card">
                    <div class="form-card__header">
                        <div class="form-card__icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><polyline points="21 8 21 21 3 21 3 8"/><rect x="1" y="3" width="22" height="5"/><line x1="10" y1="12" x2="14" y2="12"/></svg></div>
                        <div><div class="form-card__title">Archivage</div><div class="form-card__sub">Box et zone de rangement</div></div>
                    </div>
                    <div class="form-card__body">
                        <div class="fields-grid fields-grid--2">
                            <div class="field">
                                <label class="field__label">Box rangement</label>
                                <input type="text" name="box_rangement" class="field__input" placeholder="BOX-D001" value="<?= htmlspecialchars($old['box_rangement']??'') ?>">
                            </div>
                            <div class="field">
                                ...
                                <div class="field__select-wrap">
                                    <select name="zone_archive">
                                        ...
                                        <?php foreach(['Salle I','Salle II','Salle III','Salle IV','Salle V'] as $s): ?>
                                            <option value="<?= $s ?>" <?= ($old['zone_archive']??'')===$s?'selected':'' ?>><?= $s ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="toggle-group" style="margin-top:16px">
                            <label class="toggle-item"><input type="checkbox" name="is_active" <?= $old['is_active']?'checked':'' ?>><div class="toggle-switch"></div><span class="toggle-label">Agence actif</span></label>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="form-sidebar">
                <div class="floating-completion">
                    ...
                    <div class="live-preview">
                        <div class="live-preview__item">
                            ...
                            <span class="live-preview__label">Agence</span>
                ...
                <div class="form-sidebar__actions">
                    ...
                    <a href="agences.php" class="btn-cancel">
...
const requiredFields = ['nom_agence','responsable_prenom','responsable_nom','adresse','localite','departement'];
function updateCompletion() {
    ...
    // Live preview
    const responsable_nom = document.querySelector('[name=nom_agence]')?.value || '';
    const responsable_preresponsable_nom = document.querySelector('[name=prenom]')?.value || '';
    const nomProp = document.querySelector('[name=nom]')?.value || '';
    const localite = document.querySelector('[name=localite]')?.value || '';
    ...
    updatePreview('previewProp', (prenom||nomProp) ? `${prenom} ${nomProp}`.trim() : '');
    updatePreview('previewLoc', localite);
    updatePreview('previewDept', deptTxt === '— Sélectionner —' ? '' : deptTxt);
    
    // Badges validation
    const identiteOk = nom && document.querySelector('[name=numero_decision]')?.value;
    const proprietaireOk = prenom && nomProp;
    const localisationOk = document.querySelector('[name=adresse]')?.value && localite && deptEl?.value;
    
    const updateBadge = (badgeId, isOk) => {
        const badge = document.getElementById(badgeId);
        if (isOk && !badge.classList.contains('completed')) {
    ...
    updateBadge('badgeIdentite', identiteOk);
    updateBadge('badgeProprietaire', proprietaireOk);
    updateBadge('badgeLocalisation', localisationOk);
}
...
// Init avec données BDD
window.addEventListener('DOMContentLoaded', function() {
    applyDeptChange(<?= (int)($old['departement_id']??0) ?>, <?= (int)($old['arrondissement_id']??0) ?>, <?= (int)($old['district_id']??0) ?>);
    updateCompletion();
});
</script>
</body>
