...
if (!in_array($_SESSION['role'] ?? '', ['super_admin', 'admin'])) {
    header('Location: pharmacies.php?toast=permission_denied');
    exit;
}
...
try {
    ...
    $depts = $groupes = $allArrs = $allDists = [];
    $deptsWithArr = [];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!in_array($horaire, ['Jour', 'Nuit', '24h/24'])) {
        $errors['horaire'] = 'Horaire invalide';
        goto render;
    }

    try {
        $stmt = $pdo->prepare("
            INSERT INTO pharmacies_dpm
                (nom_pharmacie, prenom, nom, email, telephone_1, telephone_2,
                 adresse, quartier, departement, arrondissement_id, district_id,
                 zone_bzv, horaire, box_dossier, zone_archive, groupe_id, is_groupe, is_actif)
            VALUES
                (:nom_pharmacie, :prenom, :nom, :email, :telephone_1, :telephone_2,
...
<head>
    ...
    <title>Ajouter une pharmacie — DPM Archive</title>
    ...
    <style>
        ...
        /* ── Horaire picker (hp) — styles en dur, aucune dépendance externe ── */
        .hp-group {
            display: flex;
            gap: 8px;
            flex-wrap: wrap;
        ...
        .confirm-box__icon {
            ...
            justify-content: center;
            margin: 0 auto 16px;
            animation: shake-in .4s cubic-bezier(.34, 1.56, .64, 1)
        }
        ...
        /* Champs conditionnels localisation */
        .field-location {
            display: none
        }

        .field-location.visible {
            display: block
        }
    </style>
</head>
...
<body>
    ...
    <div class="sb-main">
        <header class="topbar">
            ...
            <h1 class="topbar__title">Ajouter une pharmacie</h1>
            <span class="topbar__meta"><?= date('d/m/Y') ?></span>
        </header>

        <div class="form-page">
            <div class="form-page__header">
                <h2 class="form-page__title">Nouvelle pharmacie</h2>
                <p class="form-page__sub">Remplissez les informations pour enregistrer une pharmacie</p>
            </div>
                    ...
                    <div>
                        ...
                        <div class="form-card">
                            <div class="form-card__header">
                                ...
                                <div>
                                    <div class="form-card__title">Identité de la pharmacie</div>
                                    <div class="form-card__sub">Nom, propriétaire et contacts</div>
                                </div>
                            </div>
                            <div class="form-card__body">
                                <div class="fields-grid">
                                    <div class="field field-full">
                                        ...
                                        <input type="text" name="nom_pharmacie"
                                            class="field__input <?= isset($errors['nom_pharmacie']) ? 'error' : '' ?>"
                                            placeholder="Ex : Pharmacie Centrale"
                                            value="<?= htmlspecialchars($old['nom_pharmacie'] ?? '') ?>" required>
                                        <?php if (isset($errors['nom_pharmacie'])): ?>
                                            <div class="field__error visible"><?= $errors['nom_pharmacie'] ?></div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <div class="fields-grid fields-grid--2" style="margin-top:16px">
                                    <div class="field">
                                        ...
                                        <input type="text" name="prenom"
                                            ...
                                            placeholder="Prénom" value="<?= htmlspecialchars($old['prenom'] ?? '') ?>"
                                            required>
                                        <?php if (isset($errors['prenom'])): ?>
                                            <div class="field__error visible"><?= $errors['prenom'] ?></div><?php endif; ?>
                                    </div>
                                    <div class="field">
                                        ...
                                        <?php if (isset($errors['nom'])): ?>
                                            <div class="field__error visible"><?= $errors['nom'] ?></div><?php endif; ?>
                                    </div>
                                    <div class="field">
                                        <label class="field__label required">Téléphone 1</label>
                                        <input type="tel" name="telephone_1"
                                            class="field__input <?= isset($errors['telephone_1']) ? 'error' : '' ?>"
                                            placeholder="+242 06 000 0000"
                                            value="<?= htmlspecialchars($old['telephone_1'] ?? '') ?>" required>
                                        <?php if (isset($errors['telephone_1'])): ?>
                                            <div class="field__error visible"><?= $errors['telephone_1'] ?></div>
                                        <?php endif; ?>
                                    </div>
                                    <div class="field">
                                        <label class="field__label">Téléphone 2</label>
                                        <input type="tel" name="telephone_2" class="field__input"
                                            placeholder="+242 05 000 0000"
                                            value="<?= htmlspecialchars($old['telephone_2'] ?? '') ?>">
                                    </div>
                                    <div class="field field-full">
                                ...
                                <div class="fields-grid">
                                    <div class="field field-full">
                                        ...
                                        <?php if (isset($errors['adresse'])): ?>
                                            <div class="field__error visible"><?= $errors['adresse'] ?></div><?php endif; ?>
                                    </div>
                                    <div class="field field-full">
                                        ...
                                        <?php if (isset($errors['quartier'])): ?>
                                            <div class="field__error visible"><?= $errors['quartier'] ?></div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <div class="fields-grid fields-grid--2" style="margin-top:16px">
                                    ...
                                    <div class="field">
                                        ...
                                        <div class="field__select-wrap">
                                            <select name="departement" id="selectDept"
                                                class="field__select <?= isset($errors['departement']) ? 'error' : '' ?>"
                                                required>
                                                <option value="">— Sélectionner —</option>
                                                <?php foreach ($depts as $d): ?>
                                                    <option value="<?= $d['id'] ?>"
                                                        <?= ($old['departement'] ?? '') == $d['id'] ? 'selected' : '' ?>>
                                                        <?= htmlspecialchars($d['libelle']) ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                        ...
                        <div class="form-card" style="margin-top:20px">
                            <div class="form-card__header">
                                ...
                                <div>
                                    <div class="form-card__title">Garde &amp; Archive</div>
                                    <div class="form-card__sub">Horaire, groupe et dossier</div>
                                </div>
                            </div>
                            <div class="form-card__body">
                                <!-- Horaire -->
                                <div class="field field-full" style="margin-bottom:20px">
                                    ...
                                    <div class="hp-group">
                                        <!-- Jour -->
                                        <button type="button"
                                            class="hp hp--jour <?= ($old['horaire'] ?? '') === 'Jour' ? 'hp--on' : '' ?>"
                                            data-val="Jour">
                                        ...
                                        <!-- Nuit -->
                                        <button type="button"
                                            class="hp hp--nuit <?= ($old['horaire'] ?? '') === 'Nuit' ? 'hp--on' : '' ?>"
                                            data-val="Nuit">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                        ...
                                        <!-- 24h/24 -->
                                        <button type="button"
                                            class="hp hp--h24 <?= ($old['horaire'] ?? '') === '24h/24' ? 'hp--on' : '' ?>"
                                            data-val="24h/24">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                <circle cx="12" cy="12" r="10" />
                                        ...
                                        <!-- Hidden input -->
                                        <input type="hidden" name="horaire" id="inputHoraire"
                                            value="<?= htmlspecialchars($old['horaire'] ?? '') ?>">
                                    </div>
                                    <?php if (isset($errors['horaire'])): ?>
                                        <div class="field__error visible"><?= $errors['horaire'] ?></div><?php endif; ?>
                                </div>
                                <div class="fields-grid fields-grid--3">
                                        ...
                                        <div class="field__select-wrap">
                                            <select name="groupe_id" class="field__select">
                                                ...
                                                <?php foreach ($groupes as $g): ?>
                                                    <option value="<?= $g['id'] ?>"
                                                        <?= ($old['groupe_id'] ?? '') == $g['id'] ? 'selected' : '' ?>>
                                                        <?= htmlspecialchars($g['groupe']) ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="field">
                                        ...
                                        <input type="text" name="box_dossier" class="field__input" placeholder="BOX-000"
                                            value="<?= htmlspecialchars($old['box_dossier'] ?? '') ?>">
                                    </div>
                                    <div class="field">
                                ...
                                <div class="toggle-group" style="margin-top:16px">
                                    <label class="toggle-item"><input type="checkbox" name="is_groupe"
                                            <?= isset($old['is_groupe']) ? 'checked' : '' ?>>
                                        <div class="toggle-switch"></div><span class="toggle-label">Chef de
                                            groupe</span>
                                    </label>
                                    <label class="toggle-item"><input type="checkbox" name="is_actif"
                                            <?= !$_POST || isset($old['is_actif']) ? 'checked' : '' ?>>
                                        <div class="toggle-switch"></div><span class="toggle-label">Pharmacie
                                            active</span>
                                    </label>
                                </div>
                            </div>
                    ...
                    <div class="form-sidebar">
                        <div class="form-card" style="padding:16px">
                            <div
                                ...
                                <span style="font-size:.78rem;font-weight:700;color:var(--green)"
                                    id="progressPct">0%</span>
                            </div>
                        ...
                        <div class="summary-card">
                            <div class="summary-card__header">
                                ...
                                <div class="summary-card__name" id="summaryName">Nom de la pharmacie</div>
                                <div class="summary-card__sub" id="summaryOwner">Propriétaire</div>
                            </div>
                            <div class="summary-card__body">
                                <div class="summary-row"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                        ...
                                        <path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0 1 18 0z" />
                                        <circle cx="12" cy="10" r="3" />
                                    </svg><span id="summaryAdresse" class="empty">Adresse</span></div>
                                <div class="summary-row"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                        stroke-width="2">
                                        <polygon points="1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6" />
                                        <line x1="8" y1="2" x2="8" y2="18" />
                                        <line x1="16" y1="6" x2="16" y2="22" />
                                    </svg><span id="summaryDept" class="empty">Département</span></div>
                                <div class="summary-row"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                        stroke-width="2">
                                        <path
                                            d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.45 2 2 0 0 1 3.59 1.27h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.85a16 16 0 0 0 6.29 6.29l.87-.88a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z" />
                                    </svg><span id="summaryTel" class="empty">Téléphone</span></div>
                                <div class="summary-row"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                        stroke-width="2">
                                        <circle cx="12" cy="12" r="10" />
                                        <polyline points="12 6 12 12 16 14" />
                                    </svg><span id="summaryHoraire" class="empty">Horaire</span></div>
                            </div>
                        </div>
                        <div class="action-card">
                            <div class="form-notif form-notif--success" id="formSuccess"><svg viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2.5">
                                    <polyline points="20 6 9 17 4 12" />
                                </svg>Enregistrement réussi !</div>
                            <div class="form-notif form-notif--error" id="formError"><svg viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2.5">
                                    <circle cx="12" cy="12" r="10" />
                                    <line x1="12" y1="8" x2="12" y2="12" />
                                    <line x1="12" y1="16" x2="12.01" y2="16" />
                                </svg>Veuillez corriger les erreurs</div>
    ...
    <div class="confirm-overlay" id="resetOverlay">
        <div class="confirm-box">
            ...
            <div class="confirm-box__msg">Tous les champs saisis seront effacés. Cette action ne peut pas être annulée.
            </div>
    ...
    <script>
        ...
        /* ── Logique conditionnelle localisation ── */
        function applyDeptChange(deptId, selectedArrId, selectedDistId) {
            ...
            if (DEPTS_WITH_ARR.includes(id)) {
                ...
                if (id === 1) fieldZone.classList.add('visible'); // Zone BZV seulement pour BZV
            } else {
        ...
        function doReset() {
            document.getElementById('pharmaForm').reset();
            applyDeptChange('');
            updatePreview();
            updateProgress();
            closeResetModal();
        ...
        function updatePreview() {
            ...
            document.getElementById('summaryName').textContent = nom || 'Nom de la pharmacie';
            document.getElementById('summaryOwner').textContent = (prenom || nomProp) ? `${prenom} ${nomProp}`.trim() : 'Propriétaire';

            const s = (id, val) => {
                const el = document.getElementById(id);
                el.textContent = val || id.replace('summary', '');
...
<?php if ($success): ?>document.getElementById('formSuccess').classList.add('visible');<?php elseif (!empty($errors)): ?>document.getElementById('formError').classList.add('visible'); <?php endif; ?>
        ...
        // Init au chargement (pour erreurs POST avec valeur précédente)
        const initDept = <?= json_encode((int) ($old['departement'] ?? 0)) ?>;
        const initArr = <?= json_encode((int) ($old['arrondissement_id'] ?? 0)) ?>;
        const initDist = <?= json_encode((int) ($old['district_id'] ?? 0)) ?>;
